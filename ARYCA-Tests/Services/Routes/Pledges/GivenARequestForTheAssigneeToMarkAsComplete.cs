...
{
	[TestFixture]
	public sealed class GivenARequestForTheAssigneeToMarkAsComplete
	{
		...
		public void Setup()
		{
			...
			using (_datacontext = DatabaseHelper.GetDatabase())
			{
				...
				_request = new UpdatePledgeStatusRequest
				{
					...
					NewStatus = PledgeStatuses.PledgeStatus.AwaitingSignOff
				};
		...
		public void ThenTheStatusHasBeenUpdatedCorrectly()
		{
			Assert.Multiple(() =>
			{
				...
				Assert.That(_updatedPledgeSubject.AssigneeCompleted, Is.True);
