...
namespace Client.ServicesBridge
{
	public class ConfigManagerBridge
	{
		...
		public async Task<ApplicationConfiguration> GetAppConfig(string jwToken)
		{
			...
			try
			{
				var apiResponse = await Task.FromResult(_genericHttpClient.GetAsyncConvertResult($"{_configUrl}/Application", false, jwToken)).Result;
		...
		{
			var added = await Task.FromResult(_genericHttpClient.PutAsync($"{_configUrl}/Application", request, false, jwToken)).Result;
